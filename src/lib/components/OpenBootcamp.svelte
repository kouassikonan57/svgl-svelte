<script lang="ts">
	import type { HTMLAttributes } from 'svelte/elements';

	export interface Props extends HTMLAttributes<SVGElement> {
		width?: number;
		height?: number;
	}

	let { width = 50, height = 50, ...rest }: Props = $props();
	const restAttrs = Object.entries(rest)
		.map(([k, v]) => `${k}="${v}"`)
		.join(' ');
	const svgContent = `<svg viewBox="0 0 70 51" xmlns="http://www.w3.org/2000/svg" class="css-12xw6g8" width="${width}" height="${height}" ${restAttrs}><ellipse cx="18.0497" cy="17.5852" rx="16.6122" ry="16.5852" fill="#0BF082"></ellipse><path fill-rule="evenodd" clip-rule="evenodd" d="M35.8644 17.5859H18.0488V34.9803H18.0496V50.1058H35.8651C40.6684 50.1058 44.5623 46.2119 44.5623 41.4086C44.5623 38.1684 42.7903 35.342 40.1626 33.8456C42.7899 32.3492 44.5615 29.523 44.5615 26.2831C44.5615 21.4798 40.6677 17.5859 35.8644 17.5859Z" fill="#047AF3"></path></svg>`;
</script>

{@html svgContent}
